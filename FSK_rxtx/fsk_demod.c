...
// Mueller-Müller TED
// ------------------------------------------------------------
static float mm_prev = 0.0f;      // x[n-1]
static float mm_half = 0.0f;      // x[n-1/2]
static float mm_mu = 0.03f;       // Loop gain (0.01–0.1)
static float mm_timer = 0.0f;     // Timer für Halbbit-Sample
...
void process_fsk_demodulation(float sample)
{
    ...
    // -----------------------------------------------------------------------
    // K. Mueller - M. Müller 1976 Timing Recovery   Halbbit-Sample erfassen
    // -----------------------------------------------------------------------

    mm_timer += 1.0f;
    if (mm_timer >= samples_per_half_bit) 
    {
        mm_timer -= samples_per_half_bit;
        mm_half = freq_filt;                    // Sample in der Mitte
    }
    ...
    // ---------------------------------------------------------
    // K. Mueller - M. Müller 1976 Digital-Timing-Recovery-Loop
    // ---------------------------------------------------------
    ...
    if (bit_timer <= 0) 
    {
        ...
        if (inverse_fsk)                            // Bit-Entscheidung
            smMode(freq_cnt < 0 ? 1 : 0);
        else 
            smMode(freq_cnt > 0 ? 1 : 0);
        freq_cnt = 0;
        mm_prev = x_now;
