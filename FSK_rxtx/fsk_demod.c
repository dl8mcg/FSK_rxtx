...
FskAmplitudes process_fsk_demod_baseband(float sample);
FskAmplitudes process_fsk_demod_center_nco(float sample);
FskAmplitudes(*smDemod)(float) = process_fsk_demod_center_nco;      // State Machine Demodulate and Detect
...
void init_filter_45bd_170sh(void)
{
...
void init_filter_50bd_85sh(void)
{
    float fs = SAMPLING_RATE;
...
void init_filter_50bd_450sh(void)
{
    float fs = SAMPLING_RATE;
...
void init_filter_200bd_340sh(void)
{
    float fs = SAMPLING_RATE;
...
void init_filter_300bd_850sh(void)
{
    float fs = SAMPLING_RATE;
...
void init_filter_1k2_1000sh(void)
{
    float fs = SAMPLING_RATE;
...
void BitClockSlowTrack0(int bit)                        // Zustand der Basisband-Demodulator-Statemachine: "0"-Bit 
{
...
void BitClockFastTrack0(int bit)
{
	bit_timer += BIT_TIMER_SCALE;                       // Bit-Timer hochzählen, skaliert mit BIT_TIMER_SCALE für die Fixkomma-Statemachine
	if (bit_timer >= samples_per_bit)                   // Abtastung der Mitte des Bits erreicht
    {
...
void BitClockFastTrack1(int bit)                        // Zustand der Basisband-Demodulator-Statemachine: "1"-Bit
{
...
void init_fsk_demod(FskMode mode)
{
    ...
    switch (mode)
    {
        case FSK_RTTY_45_BAUD_170Hz:
			...
			smDemod = process_fsk_demod_center_nco;
		...
		case FSK_AX25_9600_BAUD:
			init_filter_9k6();
            smDemod = process_fsk_demod_baseband;
            baud_rate = 9600.0f;
